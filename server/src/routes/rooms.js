...
// Создание комнаты — авторизованное (гость тоже может), ключ по userId (после requireAuth).
// Раньше комнату создавали осознанно с главной, но теперь она заводится и при заходе по прямой
// ссылке /room/:code на несуществующий код (авто-создание). Значит создание — это один заход по
// ссылке, и скрипт мог бы минтить комнаты пачками (/room/aaa1, /room/aaa2…). Крон подберёт пустые,
// но до этого забьёт БД. 20/15мин человеку с запасом, скриптовый перебор душит.
const createLimiter = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req) => String(req.user.id),
  message: { error: 'Слишком много новых комнат. Подожди немного.' },
});
...
router.post('/', requireAuth, createLimiter, createRoom); // создать комнату (+ авто-создание по прямой ссылке)
