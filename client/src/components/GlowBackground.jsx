// Фоновое «индиго-свечение» — единый тёмный вайб на всех экранах (главная, преджоин, звонок,
// секреты). Раньше этот блок копипастился по 5 страницам; теперь одна точка правки.
// variant: 'indigo' (дефолт) | 'violet' (секреты) | 'home' (два пятна на главной).
// Классы цвета заданы литералами (не через шаблон) — иначе Tailwind не увидит их при сборке.
export default function GlowBackground({ variant = 'indigo' }) {
  if (variant === 'home') {
    return (
  ...
  const color = variant === 'violet' ? 'bg-violet-600/15' : 'bg-indigo-600/15'
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
